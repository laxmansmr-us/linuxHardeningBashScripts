...
if grep -i -r "\-a always,exit \-F path=/usr/bin/chacl \-F perm=x \-F auid>=${UID_MIN} \-F auid!=unset \-k perm_chng" /etc/audit/rules.d/*; then
    ...
    echo " "
    echo "------------------------------------------------------------------------"
    echo "The Rules to collect successful and unsuccessful attempts to use the chacl command have configured already, No thing to do"
    ...
    echo " "
    echo "------------------------------------------------------------------------"
    echo "The Rules to collect successful and unsuccessful attempts to use the chacl command have NOT configured, working on set it now"
